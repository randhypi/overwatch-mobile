import 'dart:ui';

class AppColors {
  static const Color background = Color(0xFF282C34);
  static const Color cardBg = Color(0xFF323842);
  static const Color headerBg = Color(0xFF1E2127);
  static const Color primaryText = Color(0xFFABB2BF);
  static const Color secondaryText = Color(0xFF7F8899);
  static const Color inputBg = Color(0xFF3A3F4B);
  static const Color primary = Color(0xFF61AFEF);
  static const Color success = Color(0xFF98C379);
  static const Color error = Color(0xFFE06C75);
  static const Color divider = Color(0xFF5C6370);
}
